#ifndef CUSTOM_FLOWER_FORWARD_PASS_INCLUDED
#define CUSTOM_FLOWER_FORWARD_PASS_INCLUDED
...
Varyings FlowerPassVertex(Attributes input)
{
    Varyings output;
    ...
    wind.direction = lerp(defaultDir, fieldDir, step(0.01, Length2(fieldDir)));
...
half4 FlowerPassFragment(Varyings input, FRONT_FACE_TYPE facing : FRONT_FACE_SEMANTIC) : SV_Target
{
    ...
    float4 rgba = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, input.baseUV);

    clip(rgba.a - 0.5);
    
    surfaceData.albedo = rgba.rgb;
