    ...
    public class SimulationManager : IManager, IValid
    {
        ...
        struct CreateUpdateParticleList : IJobParallelFor
        {
            ...
            public void Execute(int teamId)
            {
                ...
                var tdata = teamDataArray[teamId];
                if (tdata.IsEnable == false || tdata.IsRunning == false)
                    return;
                ...
                if (parameter.triangleBendingConstraint.method != TriangleBendingConstraint.Method.None)
                {
                    ...
                    {
                        //stepTriangleBendIndexArray[start + i] = bendIndex;
                        uint pack = DataUtility.Pack10_22(teamId, bendIndex);
            ...
            public void Execute(int index)
            {
                ...
                if (attr.IsMove())
                {
                    ...
                    if (isCollision && friction > 0.0f && staticFrictionParam > 0.0f)
                    {
                        ...
                        float tangentVelocity = math.length(tanv) / tdata.SimulationDeltaTime; // 接線方向の移動速度
        ...
        public override string ToString()
        {
            ...
            sb.AppendLine($"  -Distance:{distanceConstraint?.DataCount ?? 0}");
            //sb.AppendLine($"  -SelfCollision PointPoint:{selfCollisionConstraint?.PointPointCount ?? 0}, PointTriangle:{selfCollisionConstraint?.PointTriangleCount ?? 0}");

            // 各制約
            sb.AppendLine();
            //sb.Append(selfCollisionConstraint);
