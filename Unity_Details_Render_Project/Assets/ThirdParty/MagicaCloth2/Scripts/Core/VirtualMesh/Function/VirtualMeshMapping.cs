...
namespace MagicaCloth2
{
    public partial class VirtualMesh
    {
        ...
        public void Mapping(VirtualMesh proxyMesh)
        {
            try
            {
                ...
                if (mergeChunk.IsValid)
                {
                    ...
                    calcDirectWeightJob.Run(mappingWorkData.Length);
                }
                else
                {
        ...
        [BurstCompile]
        struct Mapping_CalcDirectWeightJob : IJobParallelFor
        {
            // data
            ...
            public void Execute(int vindex)
            {
                ...
                // 処理済みセット
                var useSet = new NativeParallelHashSet<ushort>(256, Allocator.Temp);
                ...
                while (stack.IsEmpty == false)
                {
                    ...
                    // ExCostSortedList4は昇順格納なので一旦1.0から引く
                    w = 1.0f - w;
                    if (weights.Add(w, pindex) == false)
                        continue;
