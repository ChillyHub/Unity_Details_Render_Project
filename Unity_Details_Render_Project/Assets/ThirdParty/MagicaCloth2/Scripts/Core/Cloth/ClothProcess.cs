    ...
    public partial class ClothProcess
    {
        ...
        async Task BuildAsync(CancellationToken ct)
        {
            ...
            try
            {
                ...
                if (cloth.SyncCloth)
                {
                    //cloth.SyncCloth.Process.IncrementSuspendCounter();
                ...
                await Task.Run(() =>
                {
                    try
                    {
                        ...
                        if (sdata.clothType == ClothType.MeshCloth && sdata.paintMode != ClothSerializeData.PaintMode.Manual)
                        {
                            ...
                            foreach (int renderHandle in copyRenderHandleList)
                            {
                                var renderData = MagicaManager.Render.GetRendererData(renderHandle);
                                setupList.Add(renderData.setupData);
                            }
                            var ret = GenerateSelectionDataFromPaintMap(proxyMesh, clothTransformRecord, setupList, paintMapDataList, out selectionData);
                        ...
                        // セレクションデータの有無
                        //bool isValidSelection = selectionData != null && selectionData?.GetHashCode() != 0;
                        ...
                        if (clothType == ClothType.MeshCloth)
                        {
                            ...
                            for (int i = 0; i < copyRenderHandleList.Count; i++)
                            {
                                ...
                                if (isValidSelection)
                                {
                                    ...
                                    // セレクション情報から切り取りの実行
                                    renderMesh.SelectionMesh(selectionData, clothTransformRecord.localToWorldMatrix, mergin);
                                    if (renderMesh.IsError)
                                    {
                                        result = renderMesh.result;
                                        throw new MagicaClothProcessingException();
            ...
            finally
            {
                ...
                if (cloth?.SyncCloth)
                {
                    //cloth.SyncCloth.Process.DecrementSuspendCounter();
                    var sync = cloth.SyncCloth;
                    while (sync != cloth && sync != null)
                    {
        ...
        /// <param name="paintMapDataList"></param>
        /// <param name="selectionData"></param>
        /// <returns></returns>
        public ResultCode GenerateSelectionDataFromPaintMap(
            VirtualMesh vmesh,
            TransformRecord clothTransformRecord, List<RenderSetupData> setupList,
            List<PaintMapData> paintMapDataList, out SelectionData selectionData
            )
        {
            ...
            {
                // ペイントマップのチェック
                if (setupList.Count != paintMapDataList.Count)
                {
                    result.SetError(Define.Result.CreateCloth_PaintMapCountMismatch);
                    throw new MagicaClothProcessingException();
                }
                ...
                // セレクションデータバッファ作成
                int totalVertexCount = setupList.Sum(x => x.vertexCount);
                using var positionList = new NativeArray<float3>(totalVertexCount, Allocator.TempJob);
                using var attributeList = new NativeArray<VertexAttribute>(totalVertexCount, Allocator.TempJob);

                // メッシュごとにセレクションデータを作成する
                int offset = 0;
                for (int i = 0; i < setupList.Count; i++)
                {
                    var setupData = setupList[i];
                    int vcnt = setupData.vertexCount;
                    var toM = MathUtility.Transform(setupData.initRenderLocalToWorld, clothTransformRecord.worldToLocalMatrix);
                    var paintMapData = paintMapDataList[i];
                    float2 xySize = new float2(paintMapData.paintMapWidth, paintMapData.paintMapHeight);
                    using var uvs = new NativeArray<float2>(vcnt, Allocator.TempJob);
                    using var vertexs = new NativeArray<float3>(vcnt, Allocator.TempJob);
                    setupData.meshDataArray[0].GetUVs(0, uvs.Reinterpret<Vector2>());
                    setupData.meshDataArray[0].GetVertices(vertexs.Reinterpret<Vector3>());
                    using var paintData = new NativeArray<Color32>(paintMapData.paintData, Allocator.TempJob);

                    // Burstにより属性マップからセレクションデータを設定
                    // ジョブはスレッドから発行できないのでRun()で実行するがBurstの恩恵があるのでC#より十分高速になる。
                ...
                selectionData.maxConnectionDistance = vmesh.maxVertexDistance.Value;
