...
namespace MagicaCloth2
{
    ...
    public class DistanceConstraint : IDisposable
    {
        ...
        internal static ConstraintData CreateData(VirtualMesh proxyMesh, in ClothParameters parameters)
        {
            ...
            NativeMultiHashMap<int, ushort> vvMap = default;
#else
            ...
            try
            {
                ...
                var dataList = new List<ushort>(dataArryaV.Length + dataArryaH.Length);
                var distanceList = new List<float>(dataArryaV.Length + dataArryaH.Length);
                for (int i = 0; i < vcnt; i++)
                {
