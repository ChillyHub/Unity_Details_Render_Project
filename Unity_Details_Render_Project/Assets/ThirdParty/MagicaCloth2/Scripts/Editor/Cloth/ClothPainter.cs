...
namespace MagicaCloth2
{
    ...
    public class ClothPainter
    {
        ...
        static void OnGUI(SceneView sceneView)
        {
            ...
            if (Event.current.type == EventType.Repaint)
            {
                ...
                if (windata.showShape)
                    ClothEditorUtility.DrawClothEditor(editMesh, ClothEditorUtility.PaintSettings, cloth.SerializeData, true, true, true);
            }
            ...
            if (updatePoint)
            {
                UpdatePoint(t, cam, ray, showAll, windata.brushSize, windata.drawPointSize);
                repaint = true;
            }
        ...
        static void UpdatePoint(Transform ct, Camera cam, Ray ray, bool showAll, float brushSize, float pointSize)
        {
            // 表示頂点選別
            CreateDispPointList(ct, cam.transform.position, cam.transform.forward, showAll, brushSize).Complete();

            // サーフェース交差判定
            rayhit = editMesh.IntersectRayMesh(ray.origin, ray.direction, showAll, pointSize);
        }

        static JobHandle CreateDispPointList(Transform ct, float3 camPos, float3 camDir, bool showAll, float brushSize, JobHandle jobHandle = default)
        {
            dispPointList.Clear();
            ...
            // ローカルカメラ
            var localCameraDirection = ct.InverseTransformDirection(camDir);
            ...
            var job = new CreateDispPointListJob()
            {
                ...
                useBrush = rayhit.IsValid(),
                brushPosition = ct.TransformPoint(rayhit.position),
                brushSize = brushSize,
        ...
        static void ApplyPaint(ClothPainterWindowData windata)
        {
            int cnt = dispPointList.Length;
            for (int i = 0; i < cnt; i++)
            {
        ...
        static void DoWindow(int unusedWindowID)
        {
            ...
            // ポイントサイズスライダー
            windata.drawPointSize = EditorGUILayout.Slider("Draw Point Size", windata.drawPointSize, 0.001f, 0.1f);

            // ブラシサイズスライダー
            windata.brushSize = EditorGUILayout.Slider("Brush Size", windata.brushSize, 0.001f, 0.2f);

            // 形状表示
            windata.showShape = EditorGUILayout.Toggle("Show Shape", windata.showShape);

            // 全表示
            windata.BackFaceCulling = EditorGUILayout.Toggle("Back-face Culling", windata.BackFaceCulling);

            // Zテスト
            windata.zTest = EditorGUILayout.Toggle("Z Test", windata.zTest);
    ...
    public class ClothPainterWindowData : ScriptableSingleton<ClothPainterWindowData>
    {
        ...
        /// ウインドウの位置とサイズ
        /// </summary>
        public Rect windowRect = new Rect(100, 100, 400, 205);
